...
function DagreD3({width, height, edges, nodes, onClick}){
  ...
  useEffect(() => {
    ...
    nodes.forEach(function(st){ g.setNode(st.name, {label: st.name}) });
    edges.forEach((link) => {
      let name = nodes[link.source].name + "-" + nodes[link.target].name + "-" + link.type
      g.setEdge(nodes[link.source].name, nodes[link.target].name, {label: link.type}, name);
    });
  ...
  return (
    <svg className='dagre-d3' ref={(r) => nodeTree = r} width={width} height={height}>
